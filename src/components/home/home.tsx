...
import * as Styled from "../../app/Page.style";
...
interface HomeProps {
    initialYear: string;
    initialScheduleData: RaceSchedule | null;
    error?: string | null;
}
...
export function Home({
    ...
    const { data, error, isLoading } = useScheduleQuery(year, initialScheduleData);

    const {
        data: eventData,
        error: eventError,
        isLoading: eventLoading,
    } = useRaceReportQuery(selectedEventId);
    ...
    return (
        <Styled.Wrapper>
            ...
            {initialError && <ErrorMessage message={`Error: ${initialError}`} />}
            ...
            <Styled.Container>
                            ...
                            {events.map((event) => {
                                ...
                                return (
                                    <Styled.RaceCard key={event.evLink}>
                                        ...
                                        {eventError && (
                                            <ErrorMessage message={`Error: ${eventError.message}`} />
                                        )}
                                    </Styled.RaceCard>
                                );
                            })}
                ...
                {selectedEventId && eventData && (
                    <Styled.EventDetails>
                        <Styled.Subtitle>
                            Event: {eventData.report.racestrip.name}
                        </Styled.Subtitle>
                        ...
                        <Styled.PositionsContainer>
                            <Styled.PositionHeader>
                                Top 3 Positions for Qualifying
                            </Styled.PositionHeader>
